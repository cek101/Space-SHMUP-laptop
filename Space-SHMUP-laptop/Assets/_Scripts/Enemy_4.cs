...
public class Part {
    ...
    // these two fields are set automaticlaly in Start ()
    // caaching like this makes it faster and easier to find these later
    [HideInInspector] // makes field on the next line not appear in the Inspector
    public GameObject           go; // the game object of this part
    [HideInInspector]
...
/* 
* <summary>
* Enemy_4 will start offscreen and then pick a random point on screen to move too.
* Once it has arrived, it will pick another random point to move to.
...
public class Enemy_4 : Enemy { // boss?
    ...
    bool Destroyed(Part prt) {
        ...
        }                   // if prt.health is 0 or less, returns true (yes, it wasa destroyed)
            return (prt.health <= 0);
        ...
        // this will override the OnCollisionEnter that is part ofthe Enemy.cs
        void OnCollisionEnter ( Collision coll) {
            ...
            switch (other.tag) {
                case "ProjectileHero":
                    ...
                    if (prtHit.health <=0) {
                        // instead of destroying this enemy, disable the daamged part
                        prtHit.go.SetActive(false);
                    }
                    // check to see if the whole shipt is destroyed
                    ...
                    if (allDestroyed) { // if it IS completely destroyed 
                        //...tell the Main singleton that this shp was destroyed
